...
void SattumaPicTab::buildUi()
{
    ...
    // ── Left ──────────────────────────────────────────────────────────────────
    auto* left = new QWidget; left->setMaximumWidth(380);
    auto* lv   = new QVBoxLayout(left); lv->setSpacing(6);
    ...
    connect(m_btnRandom, &QPushButton::clicked, this, &SattumaPicTab::onPickRandom);
    row1->addWidget(m_btnRandom); row1->addWidget(m_chkAutoPick);
    lv->addLayout(row1);
    ...
    connect(m_btnOpenFile, &QPushButton::clicked, this, &SattumaPicTab::onOpenFile);
    row2->addWidget(m_btnOpenFile); row2->addWidget(m_chkAutoOpen);
    lv->addLayout(row2);

    m_btnOpenDir = new QPushButton("📁  Select in File's Directory");
    m_btnOpenDir->setEnabled(false);
    connect(m_btnOpenDir, &QPushButton::clicked, this, &SattumaPicTab::onSelectInDir);
    lv->addWidget(m_btnOpenDir);

    // ── ShujukoPanel (DB2) – only active when kaivo entry is loaded ───────────
    lv->addWidget(m_shujuko);
    connect(m_shujuko, &ShujukoPanel::fileSelected,
            this, &SattumaPicTab::onPreviewFromShujuko);
    ...
    rv->setContentsMargins(4,4,4,4); rv->setSpacing(4);

    auto* hdr = new QLabel("Preview");
    hdr->setAlignment(Qt::AlignCenter); hdr->setStyleSheet("font-weight: bold;");
    rv->addWidget(hdr);

    m_preview = new VideoPreviewWidget; rv->addWidget(m_preview, 1);
                                        ...
                                        const QStringList& allFiles)
{
    ...
    if (m_chkAutoPick->isChecked()) {
        if (!m_allFiles.isEmpty()) onPickRandom();
        else scanAndPick(path);
    }
}
...
void SattumaPicTab::onScanDone(bool ok, QStringList, QStringList allFiles)
{
    pbDone(m_pb); m_scanThread = nullptr;
    if (ok) { m_allFiles = allFiles; onPickRandom(); }
}

void SattumaPicTab::onOpenFile()
{
    if (!m_randomFile.isEmpty()) TV::openPath(m_randomFile);
}

void SattumaPicTab::onSelectInDir()
{
    if (!m_randomFile.isEmpty()) TV::selectInExplorer(m_randomFile);
}

void SattumaPicTab::onPreviewFromShujuko(const QString& path)
{
    m_randomFile = path; displayFile(path);
}
...
void SattumaPicTab::scanAndPick(const QString& path)
{
    ...
    connect(worker, &ScanWorker::resultReady, this, &SattumaPicTab::onScanDone);
...
void SattumaPicTab::updatePreview(const QString& path)
{
    ...
    if (TV::videoSuffixes().contains(ext)) { m_preview->play(path); return; }
    QFileIconProvider fip;
    const QIcon icon = fip.icon(QFileInfo(path));
    if (!icon.isNull())
        m_preview->showPixmap(icon.pixmap(QSize(128,128)),
                              QString("File  %1").arg(ext.isEmpty() ? "(no ext)" : ext));
    else
