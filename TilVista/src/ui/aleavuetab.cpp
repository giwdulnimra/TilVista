...
void AleaVueTab::buildUi()
{
    ...
    // kaivo DB panel (right side); shujuko lives inside SattumaPicTab
...
{
    m_imagePaths.clear(); m_allFiles.clear();
                                ...
                                const QStringList& allFiles)
{
    ...
    if (!imageFiles.isEmpty()) {
        m_imagePaths = imageFiles; m_allFiles = allFiles;
        m_lastScannedPath = path;
        m_lblStatus->setText(
            QString("✓  %1 images (cache)  –  %2").arg(imageFiles.size()).arg(path));
    } else {
        m_lblStatus->setText(QString("Loaded: %1  – scanning…").arg(path));
        scanDir(path);
    }
}

void AleaVueTab::onScanDone(bool ok, QStringList imageFiles, QStringList allFiles)
{
    pbDone(m_pb); m_btnStart->setEnabled(true); m_scanThread = nullptr;
    if (ok) {
        m_imagePaths = imageFiles; m_allFiles = allFiles;
        m_lblStatus->setText(QString("✓  %1 images found.").arg(imageFiles.size()));
...
void AleaVueTab::onScanThenOpen(bool ok, QStringList imageFiles, QStringList allFiles)
{
    pbDone(m_pb); m_btnStart->setEnabled(true); m_scanThread = nullptr;
    if (!ok || imageFiles.isEmpty()) {
        QMessageBox::warning(this, "No Images", "No image files found."); return;
    }
    m_imagePaths = imageFiles; m_allFiles = allFiles;
...
void AleaVueTab::onStartSlideshow()
{
    ...
    connect(worker, &ScanWorker::resultReady, this, &AleaVueTab::onScanThenOpen);
...
void AleaVueTab::scanDir(const QString& path)
{
    ...
    connect(thread, &QThread::started,  worker, &ScanWorker::run);
    connect(worker, &ScanWorker::progressChanged, m_pb, &QProgressBar::setValue);
    connect(worker, &ScanWorker::resultReady, this, &AleaVueTab::onScanDone);
    connect(worker, &ScanWorker::resultReady, thread, &QThread::quit);
    connect(worker, &ScanWorker::resultReady, worker, &QObject::deleteLater);
    connect(thread, &QThread::finished,       thread, &QObject::deleteLater);
    thread->start();
